...
// Import Auth dan Repositori SOS dari folder utama (Fitur Bersama)
...
class SupervisorSOSFormScreen extends StatefulWidget {
  const SupervisorSOSFormScreen({super.key});

  @override
  State<SupervisorSOSFormScreen> createState() => _SupervisorSOSFormScreenState();
}

class _SupervisorSOSFormScreenState extends State<SupervisorSOSFormScreen>
  ...
  void initState() {
    ...
    _getLocation(); // Otomatis deteksi GPS saat halaman dibuka
  }
  ...
  Future<void> _getLocation() async {
    ...
    try {
      ...
      if (permission == LocationPermission.deniedForever) {
        final shouldOpen = await SosDialogs.showPermissionPermanentlyDenied(context);
        if (shouldOpen == true) {
          _returnedFromSettings = true;
          await Geolocator.openAppSettings();
        }
        setState(() {
          _errorMessage = 'Izin lokasi diblokir permanen. Aktifkan di pengaturan aplikasi.';
          _isLoadingGps = false;
        });
        return;
      }
  ...
  // --- LOGIKA MENGIRIM DATA KE SUPABASE ---
  Future<void> _kirimSOS() async {
    ...
    try {
      // Mengambil token sesi aktif milik Supervisor
      final token = context.read<AuthProvider>().token!;

      final jenisKeadaan = _isLainnyaChecked
          ? JenisKeadaan.lainnya
          : JenisKeadaan.fromString(_categoryApiValue[_selectedCategory] ?? 'lainnya');

      // Mengirim ke Supabase menggunakan mesin asli dari folder features/sos/
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'PANGGILAN SOS SUPERVISOR',
                  style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.black87),
                  textAlign: TextAlign.center,
                ),
                const SizedBox(height: 8),
                const Text('Pilih keadaan darurat:', style: TextStyle(color: Colors.black54)),
                const SizedBox(height: 16),

                // Meminjam Widget Status GPS Petugas
                SosGpsStatus(
                ...
                // Meminjam Grid Kategori Petugas
                SosCategoryGrid(
